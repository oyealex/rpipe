    ...
    terminated(
        alt((
            context(
                "Cond::TextLenRange",
                map(
                    (terminated(opt(char('!')), (tag_no_case("len"), space1)), parse_cond_range(usize)),
            ...
            context(
                "Cond::TextLenSpec",
                map(
                    (terminated(opt(char('!')), (tag_no_case("len"), space1)), parse_cond_spec(usize)),
            ...
            context(
                "Cond::NumRange",
                map(
                    (terminated(opt(char('!')), (tag_no_case("num"), space1)), parse_cond_range(parse_num)),
            ...
            context(
                "Cond::NumSpec",
                map(
                    (terminated(opt(char('!')), (tag_no_case("num"), space1)), parse_cond_spec(parse_num)),
            ...
            context(
                "Cond::Number",
                map(
                    (terminated(opt(char('!')), tag_no_case("num")), opt(preceded(space1, parse_cond_num))),
            ...
            context(
                "Cond::RegMatch",
                map(
                    (terminated(opt(char('!')), (tag_no_case("reg"), space1)), parse_cond_reg_match),
...
pub(in crate::parse) fn parse_cond_text_all_case(input: &str) -> IResult<&str, Condition, ParserError<'_>> {
    context(
        "Cond::TextAllCase",
        map((opt(char('!')), parse_2_choice("upper", "lower")), |(not_opt, upper)| {
            Condition::new(Select::TextAllCase { upper }, not_opt.is_some())
        }),
    )
    .parse(input)
}

pub(in crate::parse) fn parse_cond_ascii(input: &str) -> IResult<&str, Condition, ParserError<'_>> {
    context(
        "Cond::Ascii",
        map((opt(char('!')), parse_2_choice("ascii", "nonascii")), |(not_opt, ascii)| {
            Condition::new(Select::Ascii { ascii }, not_opt.is_some())
        }),
    )
    .parse(input)
}

pub(in crate::parse) fn parse_cond_text_empty_or_blank(input: &str) -> IResult<&str, Condition, ParserError<'_>> {
    context(
        "Cond::TextEmptyOrBlank",
        map((opt(char('!')), parse_2_choice("empty", "blank")), |(not_opt, empty)| {
            Condition::new(Select::TextEmptyOrBlank { empty }, not_opt.is_some())
        }),
    )
    .parse(input)
}
...
mod tests {
    ...
    fn test_parse_cond_text_len_range() {
        ...
        assert_eq!(parse_cond("!len ,3 "), Ok(("", Condition::new(Select::new_text_len_range(None, Some(3)), true))));
        assert_eq!(parse_cond("!len 1, "), Ok(("", Condition::new(Select::new_text_len_range(Some(1), None), true))));
        assert_eq!(parse_cond("len , "), Ok(("", Condition::new(Select::new_text_len_range(None, None), false))));
        assert_eq!(parse_cond("!len , "), Ok(("", Condition::new(Select::new_text_len_range(None, None), true))));
    ...
    fn test_parse_cond_text_len_spec() {
        ...
        assert_eq!(parse_cond("!len 3 "), Ok(("", Condition::new(Select::TextLenSpec { spec: 3 }, true))));
    }
    ...
    fn test_parse_cond_num_range() {
        ...
        assert_eq!(parse_cond("!num "), Ok(("", Condition::new(Select::Num { integer: None }, true))));
    }

    #[test]
    fn test_parse_cond_num_spec() {
        assert_eq!(parse_cond("num 3 "), Ok(("", Condition::new(Select::NumSpec { spec: Num::from(3) }, false))));
        assert_eq!(parse_cond("!num 3 "), Ok(("", Condition::new(Select::NumSpec { spec: Num::from(3) }, true))));
        assert_eq!(parse_cond("num 3.1 "), Ok(("", Condition::new(Select::NumSpec { spec: Num::from(3.1) }, false))));
        assert_eq!(parse_cond("!num 3.1 "), Ok(("", Condition::new(Select::NumSpec { spec: Num::from(3.1) }, true))));
    }
    ...
    fn test_parse_cond_num() {
        ...
        assert_eq!(parse_cond("!num  "), Ok(("", Condition::new(Select::Num { integer: None }, true))));
        assert_eq!(parse_cond("!num integer "), Ok(("", Condition::new(Select::Num { integer: Some(true) }, true))));
        assert_eq!(parse_cond("!num float "), Ok(("", Condition::new(Select::Num { integer: Some(false) }, true))));
    }

    #[test]
    fn test_parse_cond_text_all_case() {
        assert_eq!(parse_cond("upper "), Ok(("", Condition::new(Select::TextAllCase { upper: true }, false))));
        assert_eq!(parse_cond("!upper "), Ok(("", Condition::new(Select::TextAllCase { upper: true }, true))));
        assert_eq!(parse_cond("lower "), Ok(("", Condition::new(Select::TextAllCase { upper: false }, false))));
        assert_eq!(parse_cond("!lower "), Ok(("", Condition::new(Select::TextAllCase { upper: false }, true))));
        assert!(parse_cond(" ").is_err());
    }

    #[test]
    fn test_parse_cond_ascii() {
        assert_eq!(parse_cond("ascii "), Ok(("", Condition::new(Select::Ascii { ascii: true }, false))));
        assert_eq!(parse_cond("!ascii "), Ok(("", Condition::new(Select::Ascii { ascii: true }, true))));
        assert_eq!(parse_cond("nonascii "), Ok(("", Condition::new(Select::Ascii { ascii: false }, false))));
        assert_eq!(parse_cond("!nonascii "), Ok(("", Condition::new(Select::Ascii { ascii: false }, true))));
    }

    #[test]
    fn test_parse_cond_text_empty_or_blank() {
        assert_eq!(parse_cond("empty "), Ok(("", Condition::new(Select::TextEmptyOrBlank { empty: true }, false))));
        assert_eq!(parse_cond("!empty "), Ok(("", Condition::new(Select::TextEmptyOrBlank { empty: true }, true))));
        assert_eq!(parse_cond("blank "), Ok(("", Condition::new(Select::TextEmptyOrBlank { empty: false }, false))));
        assert_eq!(parse_cond("!blank "), Ok(("", Condition::new(Select::TextEmptyOrBlank { empty: false }, true))));
    }
    ...
    fn test_parse_cond_reg_match() {
        ...
        assert_eq!(parse_cond(r"!reg '\d+' "), Ok(("", Condition::new(Select::new_reg_match(r"\d+").unwrap(), true))));
    }
}
