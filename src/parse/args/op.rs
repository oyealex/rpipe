    ...
    match args.peek() {
        Some(op) => {
            ...
            match lower_op.as_str() {
                ...
                ":drop" => parse_drop(args),
                ":keep" => parse_keep(args),
...
fn parse_drop(args: &mut Peekable<impl Iterator<Item=String>>) -> Result<Option<Op>, RpErr> {
    // if let Some(cond_str) = args.next() {
    //     match crate::parse::token::condition::parse_cond(&cond_str) {
    //         Ok((remaining, cond)) => {
    //             if !remaining.is_empty() {
    //                 Err(RpErr::UnexpectedRemaining { cmd: ":drop", arg: "condition", remaining: remaining.to_owned() })?
    //             }
    //             Ok(Some(Op::Drop(cond)))
    //         }
    //         Err(err) => {
    //             Err(RpErr::ArgParseErr { cmd: ":drop", arg: "condition", arg_value: cond_str, error: err.to_string() })?
    //         }
    //     }
    // } else {
    //     Err(RpErr::MissingArg { cmd: ":drop", arg: "condition" })
    // }
    todo!()
}

fn parse_keep(args: &mut Peekable<impl Iterator<Item=String>>) -> Result<Option<Op>, RpErr> {
    // if let Some(cond_str) = args.next() {
    //     let (remaining, cond) = crate::parse::token::condition::parse_cond(&cond_str.trim_start()).map_err(|err| RpErr::ArgParseErr {
    //         cmd: ":keep",
    //         arg: "condition",
    //         arg_value: cond_str,
    //         error: err.to_string(),
    //     })?;
    //     if !remaining.is_empty() {
    //         Err(RpErr::UnexpectedRemaining { cmd: ":keep", arg: "condition", remaining: remaining.to_owned() })?
    //     }
    //     Ok(Some(Op::Drop(cond)))
    // }else {
    //     Err(RpErr::MissingArg { cmd: ":keep", arg: "condition" })
    // }
    todo!()
}
